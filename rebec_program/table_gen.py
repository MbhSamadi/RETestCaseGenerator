...
class table_gen():
    ...
    def export_table_rebec(self, rebec):
        ...
        csv_columns = ['pre', 'transition', 'vio', 'final', 'tind', 'preind', 'vioind', 'init']
        dict_data = table
        ...
        for t in all_transitions:
            ...
            if len(pre) == 0:
                table.append({
                    ...
                    'tind': self.dfa_graph.is_transition_in_deadlock(t),
                    'preind': '',
                    'vioind': '',
                    'init': self.dfa_graph.is_transition_in_init_deadlock(t)
                })
            for p in pre:
                table.append({
                    'pre':
                    ...
                    self.dfa_graph.is_final_path(t),
                    'tind': self.dfa_graph.is_transition_in_deadlock(t),
                    'preind': self.dfa_graph.is_transition_in_deadlock(p),
                    'vioind': [] if self.dfa_graph.is_backward(t) else [
                        self.dfa_graph.is_transition_in_deadlock(u)
                        for u in self.dfa_graph.get_vio_transition(p)
                    ],
                    'init': self.dfa_graph.is_transition_in_init_deadlock(t)
                })
